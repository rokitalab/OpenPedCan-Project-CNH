...
rareCNSFile <- file.path(resultsDir, "fusion_summary_rare_cns_foi.tsv")
```
...
#' 5: Exact match a list of Rare CNS tumors fusionsgenes of interest
rareCNSFuses <- c(
  "MN1--BEND2",
  "MN1--CXXC5",
  "BRD4--LEUTX",
...
allFuseLGGHGG <- filterFusion(df = putativeOncogenicDF,
                             fuses = lggFuses,
                             genes = lgghggGenes)
allFuseRareCNS <- filterFusion(df = putativeOncogenicDF,
                               fuses = rareCNSFuses,
                               genes = rareCNSGenes)
...
```
#### Write rare CNS tumor fusions to file
```{r}

allFuseRareCNS <- allFuseRareCNS %>%
  prepareOutput(specimensUnion)

# Which fusions of interest are not present in any samples?
missingRareCNSFusion <- setdiff(rareCNSFuses, colnames(allFuseRareCNS)[-1])
# For the fusions that are not present, fill those columns with 0
allFuseRareCNS[, missingRareCNSFusion] <- 0

allFuseRareCNS %>%
  arrange(Kids_First_Biospecimen_ID) %>%
  write_tsv(rareCNSFile)  
