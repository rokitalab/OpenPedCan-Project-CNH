...
# Filter metadata based on pathology diagnosis and methylation fields and include samples that
# should be classified as high-grade glioma based on defining lesions and methylation

# Samples included on the basis of the pathology diagnosis fields
hgg_methyl_highconf <- hgg_meta %>%
  # Inclusion on the basis of CBTN harmonized pathology diagnoses
  filter((dkfz_v12_methylation_subclass %in% path_dx_list$include_methyl_dkfz & dkfz_v12_methylation_subclass_score >= 0.8) | 
           (NIH_v2_methylation_Class_mean_score %in% path_dx_list$include_methyl_nih &  dkfz_v12_methylation_subclass_score >= 0.9 & NIH_v2_methylation_Superfamily_mean_score >= 0.9)) %>%
  select(Kids_First_Biospecimen_ID, match_id, pathology_diagnosis, dkfz_v12_methylation_subclass, dkfz_v12_methylation_subclass_score, NIH_v2_methylation_Class, NIH_v2_methylation_Superfamily_mean_score, NIH_v2_methylation_Class_mean_score)
...
methyl_df <- hgg_meta %>% 
  ...
  mutate(molecular_subtype_methyl = case_when(Kids_First_Biospecimen_ID %in% hgg_methyl_highconf$Kids_First_Biospecimen_ID & dkfz_v12_methylation_subclass_score >= 0.8 ~ dkfz_v12_methylation_subclass,
                                              TRUE ~ NA_character_)) %>%
  mutate(molecular_subtype_methyl = case_when(Kids_First_Biospecimen_ID %in% hgg_methyl_highconf$Kids_First_Biospecimen_ID & is.na(molecular_subtype_methyl) ~ NIH_v2_methylation_Class,
                                              TRUE ~ molecular_subtype_methyl)) %>%
  # mutate subtypes:
  mutate(molecular_subtype_methyl = case_when(grepl("HGG_", molecular_subtype_methyl) ~ "HGNET",
                                              grepl("pedHGG_RTK", molecular_subtype_methyl) ~ "HGG, RTK",
                                              grepl("pedHGG_A|pedHGG_B", molecular_subtype_methyl) ~ "HGG, chr6 CTX",
                                              molecular_subtype_methyl == "pedHGG_MYCN" ~ "HGG, MYCN",
                                              molecular_subtype_methyl %in% c("HGAP", "AAP") ~ "HGAP",
                                              ...
                                              molecular_subtype_methyl == "IHG"  ~ "IHG",
                                              molecular_subtype_methyl == "PXA"  ~ "HGG, PXA",
                                              molecular_subtype_methyl == "GBM_CBM" ~	"GBM, CBM",
                                              molecular_subtype_methyl %in% c("GBM_MES_ATYP", "GBM_MES_TYP") ~	"GBM, MES", 
                                              molecular_subtype_methyl == "GBM_PNC" ~	"GBM, PNC",
                                              molecular_subtype_methyl %in% c("GBM_RTK1", "GBM_RTK2") ~	"GBM, RTK",
                                              TRUE ~ molecular_subtype_methyl)) %>% 
...
# add remaining methyl without the subtypes of interest, first join on match id of matched methyl event
methyl_not_subtyped <- hgg_meta %>%
  filter(experimental_strategy == "Methylation",
         !Kids_First_Biospecimen_ID %in% methyl_df$Kids_First_Biospecimen_ID_Methyl) %>%
  dplyr::rename(Kids_First_Biospecimen_ID_Methyl = Kids_First_Biospecimen_ID) %>%
  select(Kids_First_Biospecimen_ID_Methyl, Kids_First_Participant_ID, sample_id, composition, match_id) %>%
  distinct()

methyl_df_plus_not_subtyped <- methyl_df %>%
  bind_rows(methyl_not_subtyped) %>% 
  distinct()

all_data_df <-  all_data_df %>% 
  full_join(methyl_df_plus_not_subtyped, by = c("Kids_First_Participant_ID", "sample_id", "composition", "match_id"))  %>% 
...
subtype_map <- molecular_subtype_table_wt %>%
  select(Kids_First_Participant_ID, sample_id, match_id, molecular_subtype, molecular_subtype_methyl) %>%
  unique()
...
all_subtypes <- subtype_map %>%
  ...
  select(Kids_First_Biospecimen_ID, match_id, molecular_subtype)
...
select(Kids_First_Participant_ID, Kids_First_Biospecimen_ID, sample_id, match_id, molecular_subtype, molecular_subtype_methyl) %>%
  # assign based on methyl
  mutate(molecular_subtype = case_when(molecular_subtype_methyl %in% c("GBM, MES", "HGNET") ~ "HGG, H3 wildtype",
...
# update non DMG brainstem glioma dx to DMG based on clinical dx for PNOC and Mioncoseq only. 
# HGG, IDH remain as HGG
dipgs <- hgg_meta %>%
  filter(sub_cohort %in% c("PNOC", "Mioncoseq") &
           grepl("Brainstem glioma", pathology_diagnosis)) %>%
  select(Kids_First_Biospecimen_ID, pathology_diagnosis, primary_site, aliquot_id, molecular_subtype)
...
molecular_subtype_table_final <- molecular_subtype_table_w_tbc %>%
  dplyr::mutate(molecular_subtype = case_when(Kids_First_Biospecimen_ID %in% dipgs$Kids_First_Biospecimen_ID & 
                                                grepl("HGG, H3 wildtype|HGG, To be classified", molecular_subtype) ~ gsub("HGG", "DMG", molecular_subtype),
                                              TRUE ~ molecular_subtype))
  
# create final lesion + subtype file
lesions_subtypes <- all_data_df %>%
  right_join(unique(molecular_subtype_table_final[,c("match_id", "molecular_subtype")])) %>%
  unique() %>%
  arrange(match_id)
```
...
lesions_subtypes_final <- bind_rows(lesions_subtypes_RNA, lesions_subtypes_DNA, lesions_subtypes_methyl) %>%
  ...
  mutate(
    molecular_subtype_methyl = {
      vals <- na.omit(molecular_subtype_methyl)
      if (length(vals) == 0) molecular_subtype_methyl else coalesce(molecular_subtype_methyl, vals[1])
    }
  ) %>%
  ungroup() %>%
  ...
  mutate(
    across(everything(), ~ {
      vals <- na.omit(.x)
      if (length(vals) == 0) .x else coalesce(.x, vals[1])
    })
  ) %>%
  ungroup() %>%
  # fix duplicates
  ...
  mutate(
    `H3F3A_H3-3A.K28M` =
      ifelse(any(`H3F3A_H3-3A.K28M` == "yes", na.rm = TRUE),
             "yes",
             `H3F3A_H3-3A.K28M`)
  ) %>%
  ungroup() %>%
