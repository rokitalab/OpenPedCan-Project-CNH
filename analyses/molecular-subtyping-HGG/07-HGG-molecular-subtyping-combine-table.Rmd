...
molecular_subtype_table_final <- molecular_subtype_table_w_tbc %>%
  dplyr::mutate(molecular_subtype = case_when(Kids_First_Biospecimen_ID %in% dipgs$Kids_First_Biospecimen_ID & 
                                              ...
                                              TRUE ~ molecular_subtype)) %>%
  # coalesce molecular subtype methyl
  group_by(match_id) %>%
  mutate(
    molecular_subtype_methyl = {
      vals <- molecular_subtype_methyl[!is.na(molecular_subtype_methyl)]
      if (length(vals) == 0) molecular_subtype_methyl else vals[1]
    }
  ) %>%
  ungroup() %>%
  distinct()
