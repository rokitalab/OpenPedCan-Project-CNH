...
histology_cg <- histology %>%
  ...
  dplyr::mutate(cancer_group = case_when(
    ...
    grepl("^Rare CNS, ", molecular_subtype) ~ "Rare CNS tumor",
    TRUE ~ cancer_group)
    ) %>%
  ...
  # harmonize myofibroblastic tumors - all inflammatory; take care of organ later
    dplyr::mutate(short_histology = case_when(short_histology == "Myofibroblastic tumor" ~ "Inflammatory Myofibroblastic Tumor", TRUE ~ as.character(short_histology)),
