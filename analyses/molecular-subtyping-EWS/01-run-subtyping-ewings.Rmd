...
clinical <- readr::read_tsv(file.path(dataDir, "histologies-base.tsv"), guess_max = 100000) %>%
  # filter for CNS tumors only --> there are extracranial EWS samples here by mistake; BS_V42WKCSW has Primary but is CNS, keep
  filter(cohort %in% c("PBTA", "Kentucky"),
             tumor_descriptor != "Primary Tumor" | Kids_First_Biospecimen_ID == "BS_V42WKCSW")
...
all_subtypes <- ews_clinical %>%
  ...
  mutate(molecular_subtype = case_when(pathology_diagnosis == "Ewings Sarcoma" ~ "Rare CNS, EWS",
