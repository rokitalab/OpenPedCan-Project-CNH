...
rare_fusion_file <- file.path(file.path(root_dir, "analyses", "fusion-summary", "results", 
...
rare_fusions_df <- read_tsv(rare_fusion_file)
...
> **Rare CNS, MN1::BEND2**: Contain gene fusions involving 5' _MN1_ and 3' fusion partner _BEND2_.

> **Rare CNS, MN1::CXXC5**: Contain gene fusions involving 5' _MN1_ and 3' fusion partner _CXXC5_.

> **Rare CNS, FOXR2 NBL**: Over-expression and/or gene fusions in _FOXR2_

> **Rare CNS, CIC SARC**: Alterations in _CIC_, commonly fused with _NUTM1_
...
bcor_fusions <- wrangle_fusions(rare_fusions_df, "BCOR--|--BCOR", "BCOR_fusions")
...
> * Rare CNS tumors with BCOR alteration
...
> **Rare CNS, BCOR-fused** and **Rare CNS, BCOR ITD**: Median age of diagnosis less than 10 years

> **Rare CNS, MN1::CXXC5** and **Rare CNS, MN1::BEND2**: Predominantly female patients.
...
### Rare CNS, MN1::BEND2 and Rare CNS, MN1::CXXC5
...
We will call the sample with the _MN1::CXXC5_ fusion Rare CNS, MN1::CXXC5.
...
mn1_df <- all_data_df %>%
  ...
  mutate(
    molecular_subtype = case_when(
      str_detect(MN1_fusions, "MN1--BEND2") ~ "Rare CNS, MN1::BEND2",
      str_detect(MN1_fusions, "MN1--CXXC5") ~ "Rare CNS, MN1::CXXC5",
      TRUE ~ NA_character_
    )
  )
```
...
> Currently none in the embryonal biospecimen list.

```{r}
bcor_df <- all_data_df %>%
  filter(BCOR_fusions != "None") %>%
  mutate(
    molecular_subtype = case_when(
      str_detect(BCOR_fusions, "BCOR") ~ "Rare CNS, BCOR-fused",
      TRUE ~ NA_character_
    )
  )
```
...
nb_foxr2_df <- all_data_df %>%
  filter(FOXR2_fusions != "None" | FOXR2 > 3) %>%
  select(contains("ID")) %>%
  mutate(molecular_subtype = "Rare CNS, FOXR2 NBL")
```


### Rare CNS, CIC SARC

> * CNS sarcoma family tumor with _CIC_ alteration
...
If neuroblastoma samples were included on the basis of `pathology_diagnosis`, `pathology_free_text_diagnosis`, and `primary_site` (see `01-samples-to-subset`), but do not meet the criteria for _FOXR2_ alterations (Rare CNS, FOXR2 NBL), those samples should not get a subtype label. 
