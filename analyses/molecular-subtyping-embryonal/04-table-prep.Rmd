...
dicer_file <- file.path(subset_dir, "dicer1_mutations.tsv")
...
emb_dna <- relevant_clinical_data %>%
  ...
  # join with DICER1 mutation data
  left_join(dicer_df, by = "Kids_First_Biospecimen_ID") %>% rename(Kids_First_Biospecimen_ID_DNA = Kids_First_Biospecimen_ID)
  #)
...
Have added in option for DICER 1s
```{r}
...
etmr_nos_df <- all_data_df %>%
  ...
  select(contains("ID"), `DICER-1_Mutation`) %>%
  mutate(
    molecular_subtype = if_else(
      !is.na(`DICER-1_Mutation`),
      "ETMR, DICER-1",
      "ETMR, NOS"))
```
