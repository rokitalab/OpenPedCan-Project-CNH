...
* [`molecular-subtyping-PB`](https://github.com/rokitalab/OpenPedCan-Project-CNH/tree/dev/analyses/molecular-subtyping-PB)
...
rare_dir <- file.path(analyses_dir, "molecular-subtyping-Rare-CNS")
...
rare_results_file <- file.path(rare_dir, results_dir, "rare-cns-molecular-subtypes.tsv")
```
...
rare_results_df <- read_tsv(rare_results_file)
```
...
embryonal_results_df <- embryonal_results_df %>%
  ...
  mutate(integrated_diagnosis = case_when(molecular_subtype == "CNS Embryonal, NOS"~ "CNS Embryonal tumor, NEC/NOS",
                                          molecular_subtype == "Rare CNS, MN1::BEND2"~ "Rare CNS tumor, MN1::BEND2",
                                          molecular_subtype == "Rare CNS, MN1::CXXC5"~ "Rare CNS tumor, MN1::CXXC5",
                                          molecular_subtype == "Rare CNS, FOXR2 NBL" ~ "Rare CNS tumor, FOXR2 NBL",
                                          molecular_subtype == "ETMR, C19MC-altered"~ "Embryonal tumor with multilayer rosettes, C19MC-altered",
                                          molecular_subtype == "Rare CNS, BCOR ITD" ~ "Rare CNS tumor, BCOR ITD",
                                          molecular_subtype == "ETMR, NOS"~"Embryonal tumor with multilayer rosettes, NOS",
                                          TRUE ~ NA_character_), 
        short_histology = case_when(molecular_subtype %in% c("ETMR, C19MC-altered", "ETMR, NOS") ~ "ETMR",
                                    is.na(molecular_subtype) ~ "Embryonal tumor",
                                    molecular_subtype == "CNS Embryonal, NOS" ~ "Embryonal tumor",
                                    TRUE ~ "Rare CNS tumor"),
        broad_histology = case_when(is.na(molecular_subtype) ~ "Embryonal tumor",
                                    molecular_subtype %in% c("ETMR, C19MC-altered", "ETMR, NOS", "Rare CNS, FOXR2 NBL", "CNS Embryonal, NOS") ~ "Embryonal tumor", 
                                    TRUE ~ "Mixed or indeterminate tumor"),
...
ews_results_df <- ews_results_df %>%
  ...
  inner_join(select(histologies_df, all_of(join_cols)),
             by = "Kids_First_Biospecimen_ID") %>%
  mutate(integrated_diagnosis = "CNS Ewing sarcoma",
         broad_histology = "Mixed or indeterminate tumor",
         short_histology = "EWS",
         Notes = if_else(!is.na(integrated_diagnosis), "Updated via OpenPedCan subtyping", Notes))
```
...
### Handling Rare CNS
```{r}
rare_results_df <-  rare_results_df %>%
  unique() %>%
  inner_join(select(histologies_df, all_of(join_cols)),
             by = "Kids_First_Biospecimen_ID") %>%
  mutate(integrated_diagnosis = case_when(grepl("Rare CNS,", molecular_subtype) ~ molecular_subtype,
                                          TRUE ~ NA_character_),
         broad_histology = case_when(molecular_subtype == "Rare CNS, FOXR2 NBL" ~ "Embryonal tumor",
                                     TRUE ~ "Mixed or indeterminate tumor"),
         short_histology = "Rare CNS",
         Notes = if_else(!is.na(integrated_diagnosis), "Subtype based on methylation;Updated via OpenPedCan subtyping", Notes))
```
...
Compile results, replace rare tumors over old results, sort, and write to file
...
# swap rare tumors
all_swap_rare <- all_results_df %>%
  filter(!Kids_First_Biospecimen_ID %in% rare_results_df$Kids_First_Biospecimen_ID) %>%
  bind_rows(rare_results_df) %>%
